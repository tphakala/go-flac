// Package i32 provides SIMD-accelerated operations on int32 slices that back
// go-flac's integer hot loops (Rice cost search, fixed predictors, quantized
// LPC, stereo decorrelation) where the per-sample work is integer arithmetic
// and channel (de)interleaving rather than floating-point math.
//
// These kernels were originally developed in github.com/tphakala/simd (the i32
// package) and are vendored here so the codec owns its hot path end-to-end and
// can evolve the kernels without a cross-repo release round-trip. Feature
// detection still rides on github.com/tphakala/simd/cpu.
//
